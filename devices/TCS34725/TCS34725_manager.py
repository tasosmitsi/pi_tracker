...
class TCS34725(BaseSensorManager):
    Gain_t = 0
    IntegrationTime_t = 0
    ...
    def initialize(self):
        # read debug option from base class
        self.debug = self._options.get('debug', False)
        
        # Set GPIO mode
        self.INT = DigitalInputDevice(
            INT_PORT, pull_up=True, active_state=None)
        if (self.debug):
    ...
    def Enable(self):
        ...
        time.sleep(0.01)
        self.Write_Byte(self.TCS34725_ENABLE,
    ...
    def Disable(self):
        ...
        self.Write_Byte(self.TCS34725_ENABLE, reg & ~(
            self.TCS34725_ENABLE_PON | self.TCS34725_ENABLE_AEN))
    ...
    def Get_RGBData(self):
        self.C = self.Read_Word(self.TCS34725_CDATAL |
                                self.TCS34725_CMD_Read_Word)
        self.R = self.Read_Word(self.TCS34725_RDATAL |
                                self.TCS34725_CMD_Read_Word)
        self.G = self.Read_Word(self.TCS34725_GDATAL |
                                self.TCS34725_CMD_Read_Word)
        self.B = self.Read_Word(self.TCS34725_BDATAL |
                                self.TCS34725_CMD_Read_Word)
    ...
    def GetRGB888(self):
        ...
        self.RGB888 = (self.RGB888_R << 16) | (
            self.RGB888_G << 8) | (self.RGB888_B)
    ...
    def GetRGB565(self):
        ...
        self.RG565 = (((RGB565_R >> 3) << 11) | (
            (RGB565_G >> 2) << 5) | (RGB565_B >> 3)) & 0xffff
    ...
    def Get_ColorTemp(self):
        ...
        r_comp = self.R - ir
        b_comp = self.B - ir
        cct = TCS34725_CT_Coef * (float)(b_comp) / \
            (float)(r_comp) + TCS34725_CT_Offset
        return cct
    ...
    def read_data(self):
        ...
        self.GetRGB565()
        lux = self.Get_Lux()
        cct = self.Get_ColorTemp()
        lux_interrupt = self.GetLux_Interrupt(0xff00, 0x00ff)
        return {'R': self.RGB888_R, 'G': self.RGB888_G, 'B': self.RGB888_B,
                'C': self.C, 'RGB565': self.RG565, 'RGB888': self.RGB888,
                'LUX': lux, 'CT': cct, 'INT': lux_interrupt}
