...
class LPS22HB(BaseSensorManager):
    def initialize(self):
        self.LPS22HB_RESET()  # Wait for reset to complete
        # Low-pass filter disabled, output registers not updated until
        # MSB and LSB have been read, Enable Block Data Update,
        # Set Output Data Rate to 0
        self._write_byte(LPS_CTRL_REG1, 0x02)

    def shutdown(self):
        # Set Output Data Rate (ODR) to 0 → power-down mode
        buf = self._read_byte(LPS_CTRL_REG1)
        buf &= 0xF8  # clear ODR bits (ODR2:0)
    ...
    def read_data(self):
        ...
        try:
            ...
            print("LPS22HB read_data exception: ", e)
        return {'pressure_hPa': PRESS_DATA, 'temperature_C': TEMP_DATA}
